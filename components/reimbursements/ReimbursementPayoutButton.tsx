...
/** Per-staff Payout button + confirmation modal. Mirrors closers'
 *  PayoutButton — same slip-upload + note flow, pointed at the
 *  reimbursement-payouts API. Pays ALL of this staff member's currently
 *  pending claims as one batch. */
export function ReimbursementPayoutButton({
  staffId,
  staffName,
  totalCentavos,
  requestCount,
  ...
  async function uploadSlip(file: File) {
    ...
    try {
      ...
      console.error('[reimbursement-payout] slip upload failed', err);
  ...
  async function confirm() {
    if (busy) return;
    if (!slipUrl) {
      window.alert('Upload the payment slip first.');
      return;
    }
    setBusy(true);
    try {
      const res = await fetch('/api/admin/reimbursement-payouts', {
        method: 'POST',
      ...
      console.error('[reimbursement-payout] confirm failed', err);
        ...
        >
          ...
          >
            ...
            <p className="mt-1 text-[13px] text-slate-600">
              Pay <strong>{staffName}</strong> <strong className="text-emerald-700">{peso(totalCentavos)}</strong>{' '}
              covering <strong>{requestCount}</strong> pending claim{requestCount === 1 ? '' : 's'}.
            </p>
            ...
            <div className="mt-4">
              ...
              {slipUrl ? (
                <div className="flex items-center justify-between gap-2 rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-2 text-[12.5px]">
                  <span className="truncate text-emerald-800">✓ {slipName ?? 'Uploaded'}</span>
                  <button type="button" onClick={reset} className="text-[11px] font-medium text-rose-700 hover:underline">
                    Replace
                  </button>
                </div>
              ) : (
