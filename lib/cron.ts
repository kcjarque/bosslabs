...
export function verifyCronAuth(req: Request): CronAuthResult {
  ...
  // Local development remains convenient, but any deployed environment must
  // fail closed when the secret is missing. These routes send messages, sync
  // financial data, and can invoke paid AI workflows.
  if (!secret) {
