...
type Solution = {
  problem: string;
  /** Short plain action title for the brief — ≤ ~8 words, an owner reads it as
   *  the bold step (e.g. "Turn off Ads 14_24hrs now"). `fix` is the one-line
   *  how/why under it. */
  headline: string;
  fix: string; lever: string; expectedEffect: string;
};
...
const OUTPUT_DISCIPLINE_RULE =
  ...
  '- SOLUTION SHAPE: mirror the problem shape — each solution is a short bold ACTION "headline" (≤ ~8 words, e.g. "Turn off Ads 14_24hrs now", "Raise the ASC budget 20%") PLUS a one-line plain "fix" (the how/why, plain words). Same clean rhythm as the problems — never a dense run-on paragraph.\n' +
  '- WATCHLIST SHAPE: each item is a SHORT plain phrase (≤ ~12 words) an owner skims as a bullet — no stat dumps, no variable names, no ad IDs (e.g. "One proven ad stuck in Meta review", "New testimonial batch too young to judge yet").';
...
/** §3e/f/g — placement, audience, and micro-conversion-funnel breakdowns:
 *  live best-effort reads that narrow WHERE a diagnosed problem lives.
 *  Same CONTEXT status as SCOPE_RULE's other supporting data — capped at
 *  DIRECTIONAL confidence by nature (an aggregate read, not a single ad's
 *  tracked purchase count), so each can point at a lever but never stands
 *  alone as the sole basis for a cut/scale/exclude call (§0b MINIMUM-SIGNAL
 *  RULE). */
const BREAKDOWNS_RULE =
...
/** §3c — pacing/budget utilization: tells "weak results" apart from
 *  "under-delivering" (or "budget-capped and ready to scale"), joining this
 *  week's actual spend to each campaign's/ad-set's declared budget. */
const PACING_RULE =
  'PACING / BUDGET UTILIZATION (§3c) — pack.thisWeek.pacing joins this week\'s actual spend to each campaign\'s (CBO/ADVANTAGE+) or ad set\'s (ABO) declared daily budget: dailyBudgetCentavos, avgDailySpendCentavos, utilizationPct (avgDailySpend / dailyBudget, a 0..1 fraction), and two flags — underDelivering (utilizationPct < 0.7) and budgetCapped (utilizationPct >= 0.95). Read it alongside the diagnostic spine above: if results are WEAK but utilization is LOW, that is a DELIVERY problem (budget too low / fragmented / targeting too narrow to spend it) — fix the delivery lever, do NOT prescribe a creative fix for a campaign/ad set that never even spent its own budget. If a WINNER is budgetCapped, the lever is to RAISE the campaign/ad-set budget (in steps, per the scaling-velocity guardrail above) — it cannot scale further as-is. dailyBudgetCentavos/utilizationPct null means the structure/budget is unknown — do not guess one.';

/** §3d — day-of-week rhythm: DIRECTIONAL-only context (too few samples per
 *  weekday to ever be SOLID), never a per-ad cut/scale/exclude reason. */
const DAY_OF_WEEK_RULE =
  'DAY-OF-WEEK RHYTHM (§3d) — pack.context.dayOfWeek carries per-weekday (Mon..Sun) blended cppCentavos/roas/spendSharePct over the 4-week CONTEXT window (last ~28 days, ~4 samples per weekday) — NOT the single week being analyzed. Capped at DIRECTIONAL confidence BY DEFINITION (§0b minimum-signal rule — too few samples per weekday to ever be SOLID). Use it ONLY to spot RHYTHM (e.g. "weekends run ~30% pricier than weekdays") to inform pacing/scheduling framing. NEVER use it as the reason to cut, scale, or exclude any specific ad — a weekday read is account-wide context, not evidence about one ad.';
...
function normalizeSolutions(x: unknown): Solution[] {
  ...
  return x
    .map((s) => {
      const o = (typeof s === 'object' && s !== null ? s : {}) as Record<string, unknown>;
      return { problem: asStr(o.problem), headline: asStr(o.headline), fix: asStr(o.fix), lever: asStr(o.lever), expectedEffect: asStr(o.expectedEffect) };
    })
