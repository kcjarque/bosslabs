...
PACING / BUDGET UTILIZATION (§3c) — pack.thisWeek.pacing joins this week's actual spend to each campaign's (CBO/ADVANTAGE+) or ad set's (ABO) declared daily budget: dailyBudgetCentavos, avgDailySpendCentavos, utilizationPct (avgDailySpend / dailyBudget, a 0..1 fraction), and two flags — underDelivering (utilizationPct < 0.7) and budgetCapped (utilizationPct >= 0.95). If results look weak but utilization is LOW, that's a DELIVERY problem (budget too low/fragmented, targeting too narrow to spend it) — not creative; say so and point at the delivery lever, don't prescribe a creative fix for a campaign/ad set that never even spent its own budget. If a WINNER is budgetCapped, the move is to RAISE the campaign/ad-set budget (in steps — see the scaling-velocity guardrail above) — it can't scale further as-is. Null dailyBudgetCentavos/utilizationPct means the structure/budget is unknown — don't guess one.
