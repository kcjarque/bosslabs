...
type Search = {
  order?: string;
  oto?: string;
  /** HMAC token over `order` — without it (or with a bad value) we won't
   *  render the password. Generated by /api/oto on the success redirect. */
  t?: string;
};
...
export default async function VaultThankYouPage({ searchParams }: { searchParams: Search }) {
  ...
  // Only render the password to a request that carries a valid HMAC token.
  // The token comes from /api/oto's success redirect — anyone landing without
  // it (a leaked or guessed order id) sees the username + "check your email"
  // fallback instead. Email is the canonical password backup.
  const tokenValid = verifyVaultToken(order, searchParams.t);
    ...
    <>
      ...
      <main className="container-tight py-12 sm:py-20">
        <div className="mx-auto max-w-2xl">
          ...
          {/* Credentials */}
          <div className="mt-9">
            <VaultCredentialsCard hub={hub} buyerEmail={signup?.email ?? ''} tokenValid={tokenValid} />
          </div>
